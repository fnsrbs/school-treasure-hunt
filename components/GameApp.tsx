...
export default function GameApp({initialScreen}:{initialScreen:string}){
 const [pendingDiscovery,setPendingDiscovery]=useState<ReturnType<typeof scanMarker>|null>(null);const pendingRef=useRef<ReturnType<typeof scanMarker>|null>(null); const [screen,setScreen]=useState(initialScreen),[game,setGame]=useState<GameState|null>(null),[ready,setReady]=useState(false),[nickname,setNickname]=useState(''),[selected,setSelected]=useState(''),[tab,setTab]=useState('전체'),[notice,setNotice]=useState(''),[wrong,setWrong]=useState(false),[confirmNew,setConfirmNew]=useState(false),[exited,setExited]=useState(false),[saveError,setSaveError]=useState(false);const gameRef=useRef(game);gameRef.current=game;const modalRef=useRef<HTMLDivElement>(null);const wrongRef=useRef(false);
 ...
 const receive=(id:string)=>{const current=gameRef.current;if(!current||busyRef.current||wrongRef.current||pendingRef.current)return;const result=scanMarker(current,id);if(result.kind==='wrong'){wrongRef.current=true;setWrong(true);}else{pendingRef.current=result;setPendingDiscovery(result);setNotice('');}};
 ...
 {screen==='ar'&&<Panel onBack={()=>go('hint')} backLabel="힌트로" className="ar-panel"><Badge>AR 마커 인식 · {stage} / 3</Badge><h2>마커를 비춰보세요</h2>{!wrong&&<CameraScanner onMarker={receive} discovery={pendingDiscovery?.kind === 'wrong' ? undefined : pendingDiscovery?.kind} onConfirm={confirmDiscovery}/>}<p className="description center">학교에 놓인 QR 마커를 찾아보세요.</p></Panel>}
