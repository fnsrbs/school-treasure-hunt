...
export default function CameraScanner({onMarker,discovery,onConfirm}:{onMarker:(id:string)=>void;discovery?:'clue'|'found';onConfirm:()=>void}){
 const video=useRef<HTMLVideoElement>(null),callback=useRef(onMarker);callback.current=onMarker;const [error,setError]=useState(''),[retry,setRetry]=useState(0);
 ...
 {discovery&&<div className="ar-discovery" role="region" aria-label="AR 발견 확인"><div className="ar-discovery-title" role="status"><strong>{discovery==='found'?'보물을 발견했습니다!':'보물의 단서를 발견했습니다!'}</strong><span>{discovery==='found'?'눈앞에 나타난 보물을 확인해보세요.':'보물상자에 새로운 힌트가 담겨 있어요.'}</span></div><div className="ar-floating-treasure"><Treasure/><div className="ar-ground-glow"/></div><div className="ar-confirm"><p>{discovery==='found'?'확인을 누르면 보물을 획득합니다.':'다음으로를 누르면 새로운 힌트가 열립니다.'}</p><button className="gold" onClick={onConfirm}>{discovery==='found'?'확인':'다음으로'}</button></div></div>}</div>;
}
