...
export async function installLocationCamera(page,{preferClues=false}={}){
 ...
 await page.addInitScript(({markers,preferClues})=>{
  Object.defineProperty(navigator.mediaDevices,'getUserMedia',{value:async()=>{
   const game=JSON.parse(localStorage.getItem('school-treasure-hunt-v1'));
   const clue=preferClues&&game.hintStage<3&&game.clueMarkers[game.hintStage-1];
   const image=new Image();image.src=clue?'/markers/'+clue+'.png':markers[game.treasures[game.currentTreasure]];
   await image.decode();
