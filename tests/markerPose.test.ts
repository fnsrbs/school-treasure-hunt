...
test('printed location QR returns four image coordinates; blank image returns no pose',()=>{
 const require=createRequire(import.meta.url),qrRequire=createRequire(require.resolve('qrcode'));
 const {PNG}=qrRequire('pngjs');const png=PNG.sync.read(readFileSync('public/markers/SCHOOL-1-1.png'));
 const pose=decodeMarkerPose(new Uint8ClampedArray(png.data),png.width,png.height)!;
 assert.equal(pose.id,'SCHOOL-1-1');assert.equal(pose.corners.length,4);
...
test('continuous tracking dispatches once, hides on loss and reacquires same marker',async()=>{
 ...
 const require=createRequire(import.meta.url),qrRequire=createRequire(require.resolve('qrcode'));
 const {PNG}=qrRequire('pngjs'),png=PNG.sync.read(readFileSync('public/markers/SCHOOL-1-1.png'));
 ...
 try{
  ...
  const QRCode=qrRequire('qrcode');const buffer=await QRCode.toBuffer('SCHOOL-1-1',{width:720});const frame=PNG.sync.read(buffer);pixels.data=new Uint8ClampedArray(frame.data);
  ...
  assert.equal(calls,1);assert.deepEqual(seen,['SCHOOL-1-1','SCHOOL-1-1',null,'SCHOOL-1-1']);
